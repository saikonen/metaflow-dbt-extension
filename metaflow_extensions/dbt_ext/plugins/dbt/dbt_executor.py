...
class DBTExecutor():
    ...
    def project_file_paths(self):
        ...
        for component in ["model-paths", "seed-paths", "test-paths", "analysis-paths", "macro-paths"]:
            rel_path = self._project_config.get(component, [None])[0]  # dbt profile config defines the file path inside a List.
            if rel_path is None:
                continue
    ...
    def _read_dbt_artifact(self, name: str):
        artifact = os.path.join(".", self.project_dir or "", self._project_config.get("target", "target"), name)
        try:
